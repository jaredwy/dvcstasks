...
<body>
    ...
    <h1>
 	   <a href="${req.contextPath}/admin/elastic/image/configuration/viewElasticImageConfigurations.action" id="elasticConfigurationsLink" title="[@ww.text name='elastic.image.configuration.list.heading' /]">[#t]
 	       [@ww.text name='elastic.image.configuration.list.heading' /]</a>&nbsp;&rsaquo;&nbsp;${configuration.configurationName?html} [#if configuration.shippedWithBamboo]<span class="grey">[@ww.text name="elastic.image.configuration.default" /]</span>[/#if][#t]
 	</h1>
    ...
    [@ui.bambooPanel titleKey='elastic.image.configuration.view.title' ]
        ...
        [@ui.displayFooter]
            [#if configuration.disabled]
                <a id="enableElasticImageConfiguration-${configuration.id}" href="[@ww.url action='enableElasticImageConfiguration' configurationId=configuration.id returnUrl='${currentUrl}'/]">
                    [@ww.text name='global.buttons.enable'/][#t]
                </a>[#lt]
            [#else]
                <a id="startInstancesForConfig-${configuration.id}" href="[@ww.url action='prepareElasticInstances' namespace="/admin/elastic" elasticImageConfigurationId=configuration.id /]">[@ww.text name='elastic.image.configuration.start'/]</a> |
                <a id="disableElasticImageConfiguration-${configuration.id}" href="[@ww.url action='disableElasticImageConfiguration' configurationId=configuration.id returnUrl='${currentUrl}'/]">
                    [@ww.text name='global.buttons.disable'/][#t]
                </a>[#lt]
            [/#if]
            | <a href="[@ww.url action='editElasticImageConfiguration' namespace='/admin/elastic/image/configuration' configurationId='${configurationId}' returnUrl='${currentUrl}' /]">[@ww.text name='global.buttons.edit' /]</a>
            [#if activeInstanceCount == 0 && !configuration.isShippedWithBamboo()]
            | <a href="[@ww.url action='deleteElasticImageConfiguration' namespace='/admin/elastic/image/configuration' configurationId='${configurationId}' returnUrl='${currentUrl}' /]">[@ww.text name='global.buttons.delete' /]</a>
            [/#if]
        [/@ui.displayFooter]
    [/@ui.bambooPanel]
    ...
    [@dj.tabContainer headings=tabs selectedTab='${selectedTab!}']
        [@dj.contentPane labelKey='elastic.image.configuration.capabilities']
            ...
            [@ww.form id='addCapability' action='addElasticCapability.action' namespace='/admin/elastic' titleKey='agent.capability.add'
