...
<div id="queues" class="portlet" >
    <p class="headingInfo">
        [#if fn.hasAdminPermission() ]
            <a id="enableAllQueues" class="internalLink" href="[@ww.url action='configureAgents!activateAll' namespace='/admin/agent' returnUrl='${returnUrl}'/]">[@ww.text name='queue.enableAll'/]</a> |
            <a id="disableAllQueues" class="internalLink" href="[@ww.url action='configureAgents!deactivateAll' namespace='/admin/agent' returnUrl='${returnUrl}'/]">[@ww.text name='queue.disableAll'/]</a>
        [/#if]
    </p>
    <h2>
        ...
        [#list buildAgents as agent]
            ...
            <div class="queue [#if !agent.enabled] disabled[/#if]  [#if agent_index % 2 == 1][/#if]">
                <h3>
                    ...
                    [#if !agent.enabled]
                        <span class="warning">[@ww.text name='queue.agent.disabled' /]</span>
                    [/#if]
                </h3>

            [#if !(action.getCurrentlyBuilding(agent.id))?exists]
                <ol>
                    ...
                    <li class="on">
                        ...
                        <ul class="queueActions">
                            ...
                            <li id="currentlyBuildingActions">
                                ...
                                [#else]
                                    <a class="internalLink build-stop" href="${req.contextPath}/build/admin/stopPlan.action?planKey=${buildId.planKey}&amp;returnUrl=${returnUrl}" title="[@ww.text name='agent.build.cancel' /]">[@ww.text name='agent.build.cancel' /]<a/>
                                [/#if]
                            </li>
                            [/#if]
                        </ul>
                        <p class="queueStatus">
                            [#if (currentlyBuilding.buildTime)?exists && currentlyBuilding.buildTime > 0]
                                [#if currentlyBuilding.buildHangDetails?has_content]
                                    <span class="errorText">Building for ${durationUtils.getPrettyPrint(currentlyBuilding.buildTime)} (Build Hung).</span>
                                [#else]
                                    Building for ${durationUtils.getPrettyPrint(currentlyBuilding.buildTime)}.
                    ...
                    [#if fn.hasPlanPermission('READ', buildContext.planKey) ]
                        ...
                        <li class="queueItem[#if reorderedBuildKey?? && buildContext.planKey = reorderedBuildKey] reorderedBuild[/#if]">
                            ...
                            [#if currentlyBuilding?has_content && !currentlyBuilding.executableElasticImages.isEmpty()]
                                ...
                                [#if fn.hasAdminPermission() ]
                                    <a href="${req.contextPath}/admin/elastic/manageElasticInstances.action" >
                                        <img class="elasticIcon" id="${buildContext.planKey}Elastic" src="[@cp.getStaticResourcePrefix /]/images/jt/icn_elastic_cloud.gif" alt="This plan can be built in the cloud.">
                                    </a>
                            ...
                            [#if fn.hasPlanPermission('BUILD', buildContext.planKey) ]
                                <li><a class="internalLink build-stop" href="${req.contextPath}/build/admin/stopPlan.action?planKey=${buildContext.planKey}&amp;returnUrl=${returnUrl}" title="[@ww.text name='queue.delete.from' /]">[@ww.text name='queue.delete.from' /]<a/></li>
                            [/#if]
                            </ul>
                            <p class="queueStatus">
