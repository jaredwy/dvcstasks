...
${build.project.name} > ${build.parent.buildName} > #${buildSummary.buildNumber} > ${build.buildName}[#t]
[#else]
${build.project.name} > ${build.buildName} > #${buildSummary.buildNumber}[#t]
[/#if]
[/#macro]

[#-- ========================================================================================== @nc.showCommitsText --]
[#macro showCommits commits='']
[#if commits?has_content && commits.size() > 0]
--------------
Code Changes
--------------
...
[#macro showJobAndTestSummary buildSummary]
...
[#if buildSummary.successful]
    [#if totalJobCount > 1]
        [#if testSummary.totalTestCaseCount == 0]
            All ${totalJobCount} jobs passed[#lt]
        [#else]
            All ${totalJobCount} jobs passed with ${testSummary.successfulTestCaseCount} tests in total.[#lt]
        [/#if]
    [#else]
        [#if testSummary.totalTestCaseCount != 0]
