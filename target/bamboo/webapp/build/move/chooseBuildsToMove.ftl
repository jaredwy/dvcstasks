...
[#if !buildQueuesDisabled]
    [@ww.text name='build.move.warning' id="buildMoveWarning"]
        [@ww.param]<a href="[@ww.url action='configureAgents!deactivateAll' namespace='/admin/agent' returnUrl='${currentUrl}'/]">[/@ww.param]
        [@ww.param]</a>[/@ww.param]
    [/@ww.text]
...
[#if projects?has_content]
    [@ww.form action="selectBuildsToMove" namespace="/admin"
        ...
        [@ww.select labelKey='build.move.destination.project' name='existingProjectKey' toggle='true'
                    list='projects' listKey='key' listValue='name'
        ...
        [@cp.displayBulkActionSelector bulkAction=action checkboxFieldValueType='id' planCheckboxName='planIds' jobCheckboxName='selectedJobs' /]
    [/@ww.form]
[#else]
