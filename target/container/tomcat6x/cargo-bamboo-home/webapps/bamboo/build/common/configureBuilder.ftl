[#if fn.hasAdminPermission()]
    <script type="text/javascript">
        function addSharedBuilderCapabilitySubmitCallback(result) {
            ...
            $selectBuilderElement.append(AJS.$(document.createElement("option")).attr("value", result.capability.label).attr("selected", "selected").text(result.capability.label).addClass("uiSwitch" + result.builderType))
...
[@ww.select labelKey='builder.type' name='selectedBuilderKey' id="selectedBuilderKey" toggle='true'
            ...
            uiSwitch='value'
            list=uiConfigBean.builderLabelTypeMap
