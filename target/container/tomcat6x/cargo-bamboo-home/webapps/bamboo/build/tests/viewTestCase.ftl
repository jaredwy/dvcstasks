...
<div id="buildSummary">
    <h1>Test Summary</h1>
    <p>This page summarises information for <strong>${testCase.name?html}</strong> in test class <strong title="${testCase.testClass.name?html}">${testCase.testClass.shortName?html}</strong>.</p>
    [@displayDetails /]
    [@displayStats /]
</div>
...
<div id="planDetailsSummary" class="result-summary">
    <div class="details">
        <h2>Details</h2>
        <dl>
            <dt>Test Class</dt>
            ...
            [#if testCase.lastRanBuildNumber > 0]
                ...
                [#if testCase.firstRanBuildNumber > 0]
                    <dt>First ran</dt>
                    <dd>
                        <a href="[@ww.url value='/browse/${build.key}-${testCase.firstRanBuildNumber}'/]">#${testCase.firstRanBuildNumber}</a>
                    </dd>
                [/#if]
...
[#macro displayBuildInfo brs showColorCode=false hasTestCasePassed=""]
    ...
    <span class="${cssClass}">[@ps.buildResultLink plan.key brs.buildNumber/]</span>
