...
<html>
<head>
	<title>[@ui.header pageKey='buildResult.artifacts.title' object='${build.name} ${buildResultsSummary.buildNumber}' title=true /]</title>
...
<body>
    ...
    [#if fn.hasPlanPermission('ADMINISTRATION', build)]
    ...
    [@ui.bambooPanel titleKey='chain.artifacts.header' cssClass='form-view-header' auiToolbar=artifactTools headerWeight='h1' /]

    [#assign hasAdminPermission = fn.hasPlanPermission('ADMINISTRATION', plan)/]
    [@ui.bambooPanel titleKey='buildResult.artifacts.build.title' ]
        ...
        [#else]
            ...
            [/#if]
        [/#if]
        [#if !resultsSummary.active && fn.hasPlanPermission('WRITE', build)]
            [#if anyJobArtifact]
                            ...
                            <tr id="subscription-${subscription.id}">
                                <td>
                                    [#if subscription.artifactLink.exists ]
                                        [@ui.icon type="artifact-shared"/] <a  id="subscriptionName-${subscription.id}" href="${req.contextPath}${subscription.artifactLink.url}">${subscription.artifactLink.label}</a>
                                    [#else]
                                ...
                                <td>
                                    [#if subscription.artifactLink.producerJobResult??]
                                        <a id="producerJob-${subscription.consumerResultSummary.buildResultKey}" href="${req.contextPath}/browse/${subscription.artifactLink.producerJobResult.buildResultKey}/artifact">${subscription.artifactLink.producerJobResult.plan.buildName}</a>
                                        <span class="subGrey">${subscription.artifactLink.producerJobResult.plan.stage.name}</span>
                                        [#else]
