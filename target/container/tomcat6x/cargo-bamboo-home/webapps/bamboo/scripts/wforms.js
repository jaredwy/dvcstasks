/***
 ...
 Build $Sun, 23 Jul 2006 19:07:50 UTC$
...
if (wFORMS)
{
    ...
    // a hint id is the associated field id (or name) plus this suffix
    wFORMS.className_inactiveFieldHint = "field-hint-inactive";
    // visual effect depends on CSS stylesheet
        ...
        run: function(e)
        {
            ...
            if (fieldHint)
            {
                ...
                if (element.tagName.toUpperCase() == 'SELECT')
                    ...
                    fieldHint.style.left = (wFORMS.helpers.getLeft(element)).toString() + "px";
                /* + element.offsetWidth */
...
if (wFORMS)
{
    ...
    //arrMsg[4] for backward compatibility
    wFORMS.arrMsg[5] = "Previous Page";
    //arrMsg[5] for backward compatibility
        ...
        evaluate: function(node)
        {
            if (wFORMS.helpers.hasClass(node, wFORMS.className_paging))
            {
                ...
                else
                {
                    ...
                    wFORMS.helpers.addEvent(form, 'submit', function(e)
                    {
                        var element = wFORMS.helpers.getSourceElement(e);
...
if (wFORMS)
{
    ...
    // if true, Repeat behavior will preserve name attributes for radio input.
    wFORMS.limitSwitchScope = true;
    // if true, Repeat behavior will limit the scope of nested switches.
    ...
    // ------------------------------------------------------------------------------------------
        evaluate: function(node)
        {
            ...
            if (wFORMS.helpers.hasClass(node, wFORMS.className_repeat))
            {
                ...
                if (!repeatLink)
                {
                    ...
                    // could be used for CSS image replacement
        ...
        duplicateFieldGroup: function(e)
        {
            ...
            if (element)
            {
                ...
                // duplicate node tree
                var dupTree = wFORMS.behaviors['repeat'].replicateTree(element, null, suffix, preserveRadioName);
                ...
                element.parentNode.insertBefore(dupTree, insertNode);
                // Buggy rendering in IE5/Mac
        ...
        replicateTree: function(element, parentElement, idSuffix, preserveRadioName)
        {
            ...
            else if (element.nodeType == 1)
            {
                ...
                else
                {
                    ...
                    ;
                    // nail it down for IE5 ?, breaks in IE6
...
if (wFORMS)
{
    ...
    // used to keep track of the switch state on buttons and links (where the checked attribute is not available)
    ...
    // associative multi-dimensional array (switchname->element Ids)
        ...
        run: function(e)
        {
            ...
            switch (element.tagName.toUpperCase())
                ...
                case 'INPUT':
                    ...
                    break;
                default:
                    if (!wFORMS.helpers.hasClass(element, wFORMS.className_switchIsOn))
                    {
                        switches_ON = switches_ON.concat(wFORMS.behaviors['switch'].getSwitchNames(element));
                    }
                    else
        ...
        isWithinSwitchScope: function(trigger, target)
        {
            ...
            if (wFORMS.hasBehavior('repeat') && wFORMS.limitSwitchScope == true)
            {
                    ...
                    wFORMS.helpers.hasClass(scope, wFORMS.className_delete))
                {
                    ...
                    {
                        return true;
                        // target & trigger are in the same repeat/remove element
                    }
                    else
                    {
                        return false;
                        // target not in the same repeat/remove element,
                    }
                }
                else
                {
                    return true;
                    // trigger is not nested in a repeat/remove element, scope unaffected
                }
            }
            else
                return true;
...
if (wFORMS)
{
    ...
    // prevents submission when pressing the 'enter' key. Set to true if pagination behavior is used.
    wFORMS.showAlertOnError = true;
    // sets to false to not show the alert when a validation error occurs.
    ...
    // ------------------------------------------------------------------------------------------
        evaluate: function(node)
        {
            if (node.tagName.toUpperCase() == "FORM")
            {
                ...
                // @HACK temporarily hacky fix for functional test
    ...
    // ------------------------------------------------------------------------------------------
        run: function(e)
        {
            ...
            if (nbErrors > 0)
            {
                ...
                {
                    wFORMS.behaviors['validation'].showAlert(nbErrors);
                }
                return wFORMS.helpers.preventEvent(e);
            }
            return true;
                    ...
                    for (j = 0; j < arrClasses.length; j++)
                    {
                        switch (arrClasses[j])
                            ...
                            case "validate-emails":
                                if (!wBehavior.isEmails(element.value))
                                {
                                    wBehavior.showError(element, wBehavior.errMsg_email);
                                    nbErrors++;
                                }
    ...
    // ------------------------------------------------------------------------------------------
        ...
        isEmails: function(s)
        {
            ...
            {
                if (!wFORMS.behaviors['validation'].isEmail(s[i]))
                    return false;
            }
            return true;
        },
...
if (wFORMS)
{
    ...
    // used to keep track of the sync state on buttons and links (where the checked attribute is not available)
    wFORMS.className_syncIsOff = "sncIsOff";
    wFORMS.syncScopeRootTag = "";
    // deprecated.

    wFORMS.syncTriggers = {};
    // associative multi-dimensional array (syncname->element Ids)
    wFORMS.syncTargets = {};
    // associative multi-dimensional array (syncname->element Ids)
