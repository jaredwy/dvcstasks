...
[#macro displayAuthorBuildsTable buildResults totalBuildNumber=0]
    <table class="authorBuildsTable aui">
        ...
        <tbody>
            [@ww.url id='buildUrl'
                     value='/browse/${result.buildKey}-${result.buildNumber}' /]
            [@ww.url id='commitDetailUrl'
                     value='/browse/${result.buildKey}-${result.buildNumber}/commit' /]

            <tr>
                <td class="buildKeyColumn ${result.buildState}"><a href="${buildUrl}">${result.buildKey?replace("-", " &rsaquo; ")} &rsaquo; #${result.buildNumber}</a></td>
