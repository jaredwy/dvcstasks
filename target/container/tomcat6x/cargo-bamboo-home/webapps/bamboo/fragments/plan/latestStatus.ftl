...
<div id="plan-status">
    [#if plan??]
        [#assign statusSource = plan/]
        [#if plan.type == 'JOB']
            [#assign statusSource = plan.parent/]
        [/#if]
        ...
        [#if numberOfRunningBuilds gte 1 ]
            [#if numberOfRunningBuilds == 1]
                [#list action.getCurrentlyBuildingPlans(statusSource.planKey) as currentlyRunning]
                    ...
                    [#else]
                        <p class="queued">
                            [#if currentlyRunning_index == 0]<strong>[@ww.text name='build.status.current' /]:</strong>[/#if] <a class="buildLink" href="${currentBuildUrl}">#${currentlyRunning.buildIdentifier.buildNumber}</a> is queued
                        </p>
                    [/#if]
                [/#list]
            [#else]
        ...
        [#elseif statusSource.latestResultsSummary??]
            ...
            [@ww.url id='latestBuildUrl' value='/browse/${latestBuildSummary.buildResultKey}' /]
            [#if latestBuildSummary.successful]
                <p class="success">
